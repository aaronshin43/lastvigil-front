...
export class LandingScreen {
  ...
  private animate = () => {
    ...
    const ornamentAlpha = this.easeInOut(Math.max(0, Math.min((elapsed - 1000) / 800, 1))); // 1.0-1.7초: 장식+타이틀
    const buttonAlpha = this.easeInOut(Math.max(0, Math.min((elapsed - 1500) / 600, 1))); // 1.5-2.1초: 버튼
    ...
    this.draw(bgAlpha, ornamentAlpha, buttonAlpha, ornamentYOffset, buttonYOffset);
   ...
   * 랜딩 화면 그리기
   */
  draw(bgAlpha = 1, ornamentAlpha = 1, buttonAlpha = 1, ornamentYOffset = 0, buttonYOffset = 0) {
    ...
    // 1. 배경 이미지 (landing.png) - 페이드 인
    if (this.landingBg && this.landingBg.complete) {
      this.ctx.save();
      this.ctx.globalAlpha = bgAlpha;
      this.ctx.drawImage(this.landingBg, 0, 0, this.canvas.width, this.canvas.height);
      this.ctx.restore();
    }
    ...
    if (this.flourishOrnament && this.flourishOrnament.complete) {
      ...
      const ornamentHeight = (this.flourishOrnament.naturalHeight / this.flourishOrnament.naturalWidth) * ornamentWidth;
    ...
    if (this.landingTitle && this.landingTitle.complete) {
      ...
      const titleHeight = (this.landingTitle.naturalHeight / this.landingTitle.naturalWidth) * titleWidth;
    ...
    if (this.startButton && this.startButton.complete) {
      ...
      const buttonHeight = (this.startButton.naturalHeight / this.startButton.naturalWidth) * buttonWidth;
