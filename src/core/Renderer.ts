...
export class Renderer {
  ...
  constructor(config: RendererConfig) {
    // 배경 캔버스 초기화
    this.backgroundCanvas = document.getElementById(config.backgroundCanvasId) as HTMLCanvasElement;
    if (!this.backgroundCanvas) {
      throw new Error(`Canvas with id "${config.backgroundCanvasId}" not found`);
    }
    this.backgroundCtx = this.backgroundCanvas.getContext('2d')!;
    
    // 게임 객체 캔버스 초기화
    this.gameCanvas = document.getElementById(config.gameCanvasId) as HTMLCanvasElement;
    if (!this.gameCanvas) {
  ...
  private drawBackground(): void {
    ...
    const imageWidth = this.backgroundImage.naturalWidth * (imageHeight / this.backgroundImage.naturalHeight);
