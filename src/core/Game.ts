...
export class Game {
  ...
  private updateEnemies(enemyStates: EnemyStateData[]): void {
    ...
    console.log(
      `👾 Enemy update: ${enemyStates.length} enemies from server, ${this.enemies.size} in game`
    );
    ...
    for (const enemyState of enemyStates) {
      ...
      if (!enemy) {
        ...
        console.log(
          `✨ Enemy created: ${enemyState.id}, type=${
            enemyState.typeId
          }, x=${enemyState.x.toFixed(3)}, y=${enemyState.y.toFixed(3)}`
        );
      }
  ...
  private render(deltaTime: number): void {
    ...
    if (drawnEnemies > 0) {
      console.log(
        `👾 Drew ${drawnEnemies} enemies | camera offset: ${this.camera
          .getOffsetX()
          .toFixed(0)}`
      );
    }
